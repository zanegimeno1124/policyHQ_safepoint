...
export const AgentOverview: React.FC = () => {
  ...
  useEffect(() => {
    Promise.all([
      ...
      agentOverviewApi.getTeams()
    ])
      .then(([agenciesData, teamsData]) => {
  ...
  useEffect(() => {
    ...
    agentOverviewApi.getAllTimeLeaderboard(selectedAgencyId, selectedTeamId, dateRange.start, dateRange.end)
    ...
    agentOverviewApi.getSubmissionSummary(selectedAgencyId, selectedTeamId, dateRange.start, dateRange.end)
        .then(setSummary)
        .catch(console.error)
        .finally(() => setLoadingSummary(false));

    setLoadingCarriers(true);
    agentOverviewApi.getCarrierBreakdown(selectedAgencyId, selectedTeamId, dateRange.start, dateRange.end)
        .then(setCarrierBreakdown)
        .catch(console.error)
        .finally(() => setLoadingCarriers(false));
  }, [selectedAgencyId, selectedTeamId, dateRange, latestSale]);
  ...
  const selectedLabel = useMemo(() => {
    ...
    if (selectedTeamId) {
      return teams.find(t => t.id === selectedTeamId)?.label || "Global Overview";
    }
    return "Global Overview";
  }, [selectedAgencyId, selectedTeamId, agencies, teams]);
          ...
          <div className="flex items-center p-1 bg-white rounded-full border border-slate-100 shadow-[0_10px_30px_-10px_rgba(0,0,0,0.05)]">
            ...
            <div className="relative border-r border-slate-100 pr-1" ref={dropdownRef}>
                ...
                >
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-700 min-w-[100px] text-left">{selectedLabel}</span>
                ...
                {isAgencyDropdownOpen && (
                    <div className="absolute top-full left-0 w-full mt-3 w-72 bg-white rounded-[1.75rem] shadow-2xl border border-slate-100 py-3 z-50 animate-in fade-in zoom-in-95 duration-200 origin-top-left ring-1 ring-black/5">
                        ...
                        <button
                            onClick={() => { setSelectedAgencyId(null); setSelectedTeamId(null); setIsAgencyDropdownOpen(false); }}
                            className={`w-full flex items-center justify-between px-5 py-3 text-[10px] font-black uppercase tracking-widest transition-colors ${!selectedAgencyId && !selectedTeamId ? 'text-brand-600 bg-brand-50/30' : 'text-slate-500 hover:bg-slate-50'}`}
                        >
                            <span className="flex items-center gap-2"><Layers className="w-3.5 h-3.5" /> Global Overview</span>
                            {!selectedAgencyId && !selectedTeamId && <Check className="w-3.5 h-3.5" strokeWidth={4} />}
                        </button>

                        <div className="h-px bg-slate-100 my-2 mx-4"></div>

                        <div className="px-5 py-2 mb-2">
                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Agencies</p>
                        </div>
                        <div className="max-h-32 overflow-y-auto px-1 mb-2">
                            {agencies.map(agency => (
                                <button
                                    key={agency.id}
                                    onClick={() => { setSelectedAgencyId(agency.id); setSelectedTeamId(null); setIsAgencyDropdownOpen(false); }}
                                    className={`w-full flex items-center justify-between px-5 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${selectedAgencyId === agency.id && !selectedTeamId ? 'text-brand-600 bg-brand-50/30' : 'text-slate-500 hover:bg-slate-50'}`}
                                >
                                    <span className="truncate">{agency.label}</span>
                                    {selectedAgencyId === agency.id && !selectedTeamId && <Check className="w-3.5 h-3.5" strokeWidth={4} />}
                                </button>
                            ))}
                        </div>
                        ...
                        <div className="px-5 py-2 mb-2">
                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Teams</p>
                        </div>
                        <div className="max-h-32 overflow-y-auto px-1">
                            {teams.map(team => (
                                <button
                                    key={team.id}
                                    onClick={() => { setSelectedTeamId(team.id); setSelectedAgencyId(null); setIsAgencyDropdownOpen(false); }}
                                    className={`w-full flex items-center justify-between px-5 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${selectedTeamId === team.id && !selectedAgencyId ? 'text-brand-600 bg-brand-50/30' : 'text-slate-500 hover:bg-slate-50'}`}
                                >
                                    <span className="truncate">{team.label}</span>
                                    {selectedTeamId === team.id && !selectedAgencyId && <Check className="w-3.5 h-3.5" strokeWidth={4} />}
                                </button>
                            ))}
                        </div>
                    </div>
